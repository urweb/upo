...
functor Make(M : sig
    ...
    type a = {Calendar : FullCalendar.t,
              ...
              Settable : list (keyT * source (option time)),
    ...
    val create =
        ...
        settable <- List.mapPartialM (fn r => if r.MayEdit then s <- source r.T.tm; return (Some (r.T.key, s)) else return None) ts;
                                                       ...
                                                       {Header = <xml>
                                                                         ...
                                                                         | Some ctx =>
                                                                           ...
                                                                           return (case rendering of
                                                                                     ...
                                                                                     | _ => Ui.modalIcon ctx (CLASS "glyphicon glyphicon-pencil-alt float-right")
                                                                                                         (choices <- List.mapM (fn (k, tms) =>
                                                                                                                                   tmo <- get tms;
                                                                                                                                   s <- source (tmo = Some tm);
                                                                                                                                   return (k, tms, s)) settable;
                                                                                                          return (Ui.modal
                                                                                                                      (List.app (fn (k, tms, s) =>
                                                                                                                                    b <- get s;
                                                                                                                                    if b then
                                                                                                                      ...
                                                                                                                      <xml><ul class="list-group">
                                                                                                                        {List.mapX (fn (k, _, s) => <xml><li class="list-group-item">
                                                                                                                          <ccheckbox source={s}/> {[k]}
                                                                                                                         </li></xml>) choices}
                                                                                                                      </ul></xml>
                                                                                                                      <xml>Save</xml>)))}/>
                                                        </xml>,
                                                        Body = <xml>
                                                          <active code={tm <- FullCalendar.eventStart ev;
                                                                        return <xml>
                                                                          <dyn signal={settables <- List.mapPartialM (fn (k, s) =>
                                                                                                                         tmo <- signal s;
    ...
    fun render ctx self = <xml>
      <dyn signal={unsched <- List.mapPartialM (fn (k, tms) =>
                                                   tmo <- signal tms;
                                                   return (case tmo of
                                                               None => Some k
    ...
    fun notification _ self = <xml>
      <dyn signal={unplaced <- List.existsM (fn (_, tms) => tmo <- signal tms; return (Option.isNone tmo)) self.Settable;
